...
Composition parse_comp_string(const std::string& ss, const std::vector<std::string>& names)
{
    Composition x;
    ...
    while (stop < ss.size()) {
        ...
        TORCH_CHECK(get_value(x, name, 0.0) == 0.0, fmt::format("duplicate key {}", name));

        x[name] = value;
        start = ss.find_first_not_of(", ;\n\t", stop+1);
        left = start;
    }
    TORCH_CHECK(left == start, fmt::format("Unable to parse key-value pair '{}'", ss.substr(start, stop)));
    if (stop != npos && !ba::trim_copy(ss.substr(stop)).empty()) {
      TORCH_CHECK(false, fmt::format("Found non-key:value data in composition string '{}'", ss.substr(stop)));
    }
    return x;
}
