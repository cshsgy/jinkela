...
double fp_value_check(const string& val) {
  ...
  ch = str[0];
  if (ch == '+' || ch == '-') {
    TORCH_CHECK(str.size() > 1,
                fmt::format("string '{}' ends in '{}'", val, ch));
    istart = 1;
  }
  for (size_t i = istart; i < str.size(); i++) {
    ...
    if (isdigit(ch)) {
      ...
      TORCH_CHECK(
          numDot <= 1,
          fmt::format("string '{}' has more than one decimal point.", val));
      TORCH_CHECK(
          numExp == 0,
          fmt::format("string '{}' has decimal point in exponent", val));
    } else if (ch == 'e' || ch == 'E' || ch == 'd' || ch == 'D') {
      numExp++;
      str[i] = 'E';
      if (numExp > 1) {
        TORCH_CHECK(false,
                    fmt::format("string '{}' has more than one exp char", val));
      } else if (i == str.size() - 1) {
        TORCH_CHECK(false, fmt::format("string '{}' ends in '{}'", val, ch));
      }
      ch = str[i + 1];
      if (ch == '+' || ch == '-') {
        TORCH_CHECK(i + 1 < str.size() - 1,
                    fmt::format("string '{}' ends in '{}'", val, ch));
        i++;
      }
    } else {
