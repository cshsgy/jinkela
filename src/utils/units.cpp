...
Units::Units(const std::string& name, bool force_unity)
{
    ...
    if (matched.size()) {
        ...
        if (name.find(factor) == 0) {
            m_factor = fp_value_check(factor);
            start = factor.size();
        }
    }
            ...
            *this *= knownUnits.at(unit).pow(exponent);
            ...
            if (prefixes.find(prefix) != prefixes.end() &&
                ...
                *this *= u.pow(exponent);
                ...
                throw std::runtime_error("Unknown unit '" + unit + "' in unit string '" + name + "'");
            }
        }
    ...
    if (force_unity && (std::abs(m_factor - 1.) > SmallNumber)) {
        throw std::runtime_error("Detected non-unity conversion factor:\n"
            "input '" + name + "' is equivalent to '" + str() + "' where the conversion factor is '" + std::to_string(m_factor) + "'");
    }
}
...
string Units::str(bool skip_unity) const
{
    ...
    for (auto const& [dimension, exponent] : dims) {
        ...
        if (exponent == 0.) {
            ...
            den.append(" / " + dimension);
        } else if (exponent == rounded && rounded > 0) {
            num.append(" * " + dimension + "^" + std::to_string(rounded));
        } else if (exponent == rounded) {
            den.append(" / " + dimension + "^" + std::to_string(-rounded));
        } else if (exponent > 0) {
            num.append(" * " + dimension + "^" + std::to_string(exponent));
        } else {
            den.append(" / " + dimension + "^" + std::to_string(-exponent));
        }
    }

    if (skip_unity && (std::abs(m_factor - 1.) < SmallNumber)) {
        if (num.size()) {
            return num.substr(3) + den;
        }
    ...
    if (num.size()) {
        ...
        return factor + " " + num.substr(3) + den;
    }
...
double Units::dimension(const std::string& primary) const
{
    if (primary == "mass") {
        ...
        throw std::runtime_error("Unknown primary unit '" + primary + "'.");
    }
}
...
void UnitStack::setStandardUnits(Units& standardUnits)
{
    ...
    if (stack[0].second != 0.) {
        throw std::runtime_error("Error in UnitStack::setStandardUnits: Standard unit is already defined.");
    }
...
{
    if (!stack.size()) {
        throw std::runtime_error("Error in UnitStack::join: Standard unit is not defined.");
    }
...
std::map<std::string, std::string> UnitSystem::defaults() const
{
    ...
    if (!m_defaults.count("activation-energy")) {
        units["activation-energy"] = units["energy"] + " / " + units["quantity"];
    }
...
void UnitSystem::setDefaults(std::initializer_list<std::string> units)
{
    for (const auto& name : units) {
        ...
        if (unit.convertible(knownUnits.at("kg"))) {
            ...
            if (unit.factor() != 1.) {
                throw std::runtime_error("Error in UnitSystem::setDefaults: Temperature scales "
            ...
            if (unit.factor() != 1.) {
                throw std::runtime_error("Error in UnitSystem::setDefaults: Current scales "
                    "with non-unity conversion factor from Ampere are not supported.");
            }
        } else {
            throw std::runtime_error("Error in UnitSystem::setDefaults: Unable to match unit '" + name + "' to a basic dimension");
        }
    }
...
void UnitSystem::setDefaults(const std::map<std::string, std::string>& units)
{
    for (const auto& [dimension, name] : units) {
        ...
        if (dimension == "mass" && unit.convertible(knownUnits.at("kg"))) {
            ...
            throw std::runtime_error("Error in UnitSystem::setDefaults: Unable to set default unit for '" + dimension + "' to '" + name + "' (" + unit.str() + ").");
        }
    }
...
void UnitSystem::setDefaultActivationEnergy(const std::string& e_units)
{
    ...
    if (u.convertible(Units("J/kmol"))) {
        ...
        throw std::runtime_error("Error in Units::setDefaultActivationEnergy: Unable to match unit '" + e_units + "' to a unit of activation energy");
    }
    m_explicit_activation_energy = true;
}

double UnitSystem::convert(double value, const std::string& src, const std::string& dest) const
{
                           ...
                           const Units& dest) const
{
    if (!src.convertible(dest)) {
        throw std::runtime_error("Error in UnitSystem::convert: Incompatible units:\n    Units(" + src.str() + ") and\n    Units(" + dest.str() + ")");
    }
...
static std::pair<double, std::string> split_unit(const AnyValue& v) {
    if (v.is<std::string>()) {
        ...
        if (space == npos) {
            throw std::runtime_error("Error in split_unit (UnitSystem): Couldn't parse '" + val_units + "' as a space-separated value/unit pair");
        }
        return {fp_value_check(val_units.substr(0, space)),
...
double UnitSystem::convertRateCoeff(const AnyValue& v, const Units& dest) const
{
    ...
    throw std::runtime_error("Error in UnitSystem::convertRateCoeff: Unable to convert value with non-default units to undefined units,\n"
        "likely while creating a standalone ReactionRate object.");
}

std::vector<double> UnitSystem::convert(const std::vector<AnyValue>& vals,
                                           ...
                                           const std::string& dest) const
{
    ...
    if (usrc.convertible(Units("J/kmol"))) {
        ...
        throw std::runtime_error("Error in UnitSystem::convertActivationEnergy: Don't understand units '" + src + "' as an activation energy");
    }
    ...
    if (udest.convertible(Units("J/kmol"))) {
        ...
        throw std::runtime_error("Error in UnitSystem::convertActivationEnergy: Don't understand units '" + dest + "' as an activation energy");
    }

    return value;
}

double UnitSystem::convertActivationEnergyTo(double value, const std::string& dest) const
{
                                             ...
                                             const Units& dest) const
{
    if (dest.convertible(Units("J/kmol"))) {
        ...
        throw std::runtime_error("Error in UnitSystem::convertActivationEnergyTo: '" + dest.str() + "' is not a unit of activation energy");
    }
}

double UnitSystem::convertActivationEnergyFrom(double value, const std::string& src) const
{
    Units usrc(src);
    if (usrc.convertible(Units("J/kmol"))) {
        ...
        throw std::runtime_error("Error in UnitSystem::convertActivationEnergyFrom: '" + src + "' is not a unit of activation energy");
    }
}

double UnitSystem::convertActivationEnergy(const AnyValue& v, const std::string& dest) const
{
    try {
        auto [value, units] = split_unit(v);
        if (units.empty()) {
        ...
        throw std::runtime_error("Error in UnitSystem::convertActivationEnergy: " + err.what());
    }
}
