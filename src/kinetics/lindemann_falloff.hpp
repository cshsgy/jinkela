...
//! Options for Lindemann falloff reactions: k = k0*[M]_eff / (1 + Pr)
struct LindemannFalloffOptionsImpl {
  static std::shared_ptr<LindemannFalloffOptionsImpl> create() {
    return std::make_shared<LindemannFalloffOptionsImpl>();
  }

  static std::shared_ptr<LindemannFalloffOptionsImpl> from_yaml(
      const YAML::Node& node,
      std::shared_ptr<LindemannFalloffOptionsImpl> derived_type_ptr = nullptr);
...
class LindemannFalloffImpl : public torch::nn::Cloneable<LindemannFalloffImpl> {
 public:
  ...
  LindemannFalloffOptions options;

  //! Constructor to initialize the layer
  LindemannFalloffImpl() : options(LindemannFalloffOptionsImpl::create()) {}
  explicit LindemannFalloffImpl(LindemannFalloffOptions const& options_);
  void reset() override;
