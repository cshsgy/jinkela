#pragma once
// H2 <-> 2H equilibrium folded into the thermodynamics of ONE lumped H/He species.
//
// WHY: snapy's `moist-mixture` EOS delegates every thermodynamic question to kintera
// ("VT->P", "VU->T", "VT->cv", eval_czh, ...). So a dissociating gas can be represented WITHOUT
// advecting H: kintera resolves the equilibrium internally at (T, c) and reports the resulting
// particle count, internal energy and heat capacities. No extra tracers, no chemistry operator,
// no stiff solve -- and snapy needs NO change at all.
//
// The lumped species has composition {H: nH, He: nHe} per mole. At (T, c):
//
//   Kc(T) = Kp(T) * P0/(R T)        Kp from NASA-9 Gibbs:  ln Kp = -(2 g_H - g_H2)
//   [H]   = (-Kc + sqrt(Kc^2 + 8 Kc nH c)) / 4        <- closed form, ONE quadratic (~10 flops)
//   [H2]  = (nH c - [H]) / 2 ,   [He] = nHe c
//
// and then, ALL DERIVED FROM THAT SAME SPECIATION so they are mutually consistent:
//
//   cz   = n_tot / c                     <- particles per mole of species (>1 once dissociated)
//   e/R  = sum_i n_i (h_i/R - T) / c     <- NASA-9 h is ABSOLUTE, so the 436 kJ/mol lands here
//   cv/R = [sum_i n_i cv_i/R + sum_i (u_i/R) dn_i/dT] / c   <- the 2nd term IS the latent heat
//   cp/R = cv/R + (cz + T dcz/dT)^2 / (cz + c dcz/dc)       <- exact reacting-gas Mayer relation
//
// Consistency is the whole point: grad_ad is EMERGENT from (P, u, cv), not imposed. Correcting cp
// alone is NOT enough -- the particle-number term (delta = 1.13 in the BD deep) carries ~12% of
// grad_ad, so a cp-only fix lands on 0.162 instead of the true 0.197 (fixed 2/7 gives 0.286).
...
constexpr double kTref = 300.0;  // MUST match kNasa9Tref in eval_uhs.cpp (energy continuity)
...
//! h/R [K] -- ABSOLUTE (a7 carries the formation enthalpy), so 2 h_H - h_H2 = D(T).
...
//! Everything the model needs at one (T, c). `ab` is (2,3,9) = [low|high][H2,H,He][coeff].
struct State {
...
inline State speciate(torch::Tensor const& temp, torch::Tensor const& cc, double nH,
  ...
  s.U = (s.H * (s.hH - temp) + s.H2 * (s.hH2 - temp) + s.He * (s.hHe - temp)) / cc;
  return s;
}
...
//! \param temp (...) [K]   \param c (...) molar conc. of the lumped species [mol/m^3]
inline Result eval(torch::Tensor const& temp, torch::Tensor const& c, double nH, double nHe,
  ...
  auto cv_R = (s.H * (s.cpH - 1.0) + s.H2 * (s.cpH2 - 1.0) + s.He * (s.cpHe - 1.0) +
               uH * dH_dT + uH2 * (-dH_dT / 2.0)) / cc;

  auto num = (cz + temp * dcz_dT).pow(2);
  auto den = (cz + cc * dcz_dc).clamp_min(1e-8);
  auto cp_R = cv_R + num / den;

  // reference the internal energy to kTref at the SAME c (eval_nasa9's convention -> continuity)
